...
export const crawl4aiCheckCrawlStatusSchema = {
  ...
  includeStats: z.boolean().optional().default(true)
    .describe('Include detailed statistics about the crawl (pages crawled, errors, rate limits, etc.). Provides comprehensive metrics about the crawl operation'),
  ...
  includeUrls: z.boolean().optional().default(true)
    .describe('Include the list of URLs that were successfully crawled. Useful for verification and for creating reports of crawled content'),
  
  // Optional with default: Whether to include error details in the response
  includeErrors: z.boolean().optional().default(true)
    .describe('Include details about any errors encountered during the crawl. Useful for diagnosing issues with specific pages or domains'),
  
  // Optional with default: Whether to download the crawl results
  downloadResults: z.boolean().optional().default(false)
    .describe('Download the full crawl results. If false, only the status and statistics are returned. Setting to true returns the complete crawled content'),
  
  // Optional with default: Format for downloaded results
  downloadFormat: z.enum(['json', 'markdown', 'csv']).optional().default('json')
    .describe('Format for downloaded results, if downloadResults is true. "json" is structured for programmatic use, "markdown" for human readability, and "csv" for tabular data'),
  
  // Optional with default: Maximum size in KB for downloaded results
  maxResultSize: z.number().optional().default(1000)
    .describe('Maximum size in KB for downloaded results. Larger results will be truncated. Increase for more comprehensive results, but be aware of potential memory usage'),
