...
const ScrapeOptionsSchema = z
  .object({
    ...
    formats: z
      ...
      .default(['markdown'])
      .describe(
        'Content formats to extract from each crawled page. Multiple formats can be requested simultaneously'
      ),
    ...
    onlyMainContent: z
      ...
      .default(true)
      .describe(
        'Extract only the main content from pages, filtering out navigation, sidebars, footers, etc. Improves content quality and reduces noise'
      ),
    ...
    includeTags: z
      ...
      .optional()
      .describe(
        'HTML tags to specifically include in extraction (e.g., ["article", "main", "p"]). Useful for targeting specific content types'
      ),
    ...
    excludeTags: z
      ...
      .optional()
      .describe(
        'HTML tags to exclude from extraction (e.g., ["nav", "footer", "aside"]). Helps filter out non-content elements'
      ),
    ...
    waitFor: z
      ...
      .default(2000)
      .describe(
        'Time in milliseconds to wait for dynamic content to load on each page. Increase for JavaScript-heavy sites'
      ),
    ...
    removeBase64Images: z
      ...
      .default(true)
      .describe(
        'Remove base64 encoded images from output to reduce response size. Set to false if image data is important'
      ),
  })
  .describe(
    'Options for scraping each page during the crawl. Controls content extraction behavior'
  );
...
const WebhookSchema = z
  .union([
    ...
    z
      .string()
      .describe(
        'Webhook URL to notify when crawl is complete. Will receive a POST request with crawl results'
      ),
    ...
    z
      .object({
        ...
        headers: z
          .record(z.string())
          .describe(
            'Custom headers for webhook requests. Useful for authentication or request routing'
          ),
      })
      .describe(
        'Webhook configuration with custom headers for more complex notification requirements'
      ),
  ])
  .optional()
  .describe(
    'Optional webhook to notify when the crawl is complete. Enables asynchronous processing of crawl results'
  );
...
export const crawl4aiCrawlSchema = z.object({
  ...
  url: z
    .string()
    .describe(
      'Starting URL for the crawl. The crawler will begin from this page and follow links. Must be a valid URL including protocol (e.g., https://example.com)'
    ),
  ...
  limit: z
    ...
    .default(50)
    .describe(
      'Maximum number of pages to crawl (1-1000). The crawl will stop after processing this many pages. Higher values provide more comprehensive coverage but increase processing time'
    ),
  ...
  maxDepth: z
    ...
    .default(3)
    .describe(
      'Maximum link depth to crawl (1-10). A depth of 1 means only crawl the starting page, 2 includes pages linked from the starting page, etc. Higher depths reach more content but increase processing time'
    ),
  ...
  includePaths: z
    ...
    .optional()
    .describe(
      'Only crawl URLs with these path patterns (e.g., ["/blog/", "/docs/"]). If not specified, all paths are included. Useful for targeting specific sections of a website'
    ),
  ...
  excludePaths: z
    ...
    .optional()
    .describe(
      'URL paths to exclude from crawling (e.g., ["/contact/", "/login/"]). Useful for avoiding irrelevant or sensitive pages'
    ),
  ...
  allowExternalLinks: z
    ...
    .default(false)
    .describe(
      'Allow crawling links to external domains. By default, only links within the same domain are crawled. Setting to true enables cross-domain crawling'
    ),
  ...
  allowBackwardLinks: z
    ...
    .default(true)
    .describe(
      'Allow crawling links that point to parent directories. Setting to false creates a forward-only crawl pattern'
    ),
  ...
  ignoreQueryParameters: z
    ...
    .default(true)
    .describe(
      'Ignore query parameters when comparing URLs to avoid crawling the same page with different parameters. Setting to false treats URLs with different query parameters as distinct'
    ),
  ...
  deduplicateSimilarURLs: z
    ...
    .default(true)
    .describe(
      'Remove similar URLs during crawl to avoid content duplication. Uses fuzzy matching to identify and filter out near-duplicate URLs'
    ),
  ...
  ignoreSitemap: z
    ...
    .default(false)
    .describe(
      'Skip sitemap.xml discovery. By default, sitemap.xml is used to discover URLs in addition to HTML links. Setting to true relies solely on HTML link discovery'
    ),

  // Optional: Options for scraping each page
  scrapeOptions: ScrapeOptionsSchema.optional().describe(
    'Options for scraping each page during the crawl. Controls content extraction behavior and format'
  ),
 ...
 */
export type Crawl4aiCrawlHandler = (
  params: z.infer<typeof crawl4aiCrawlSchema>
