...
export const crawl4aiMapSchema = {
  ...
  limit: z.number().min(1).max(500).optional().default(100)
    .describe('Maximum number of URLs to return (1-500). Higher values provide more comprehensive coverage but increase processing time'),
  
  // Optional: Search term to filter URLs
  search: z.string().optional()
    .describe('Optional search term to filter URLs. Only URLs containing this term will be included in the results. Case-insensitive matching is used'),
  
  // Optional with default: Whether to skip sitemap.xml discovery
  ignoreSitemap: z.boolean().optional().default(false)
    .describe('Skip sitemap.xml discovery and only use HTML links for URL discovery. By default, both methods are used for maximum coverage'),
  
  // Optional with default: Whether to only use sitemap.xml for discovery
  sitemapOnly: z.boolean().optional().default(false)
    .describe('Only use sitemap.xml for discovery, ignore HTML links. This is faster but may miss pages not listed in the sitemap'),
  
  // Optional with default: Whether to include URLs from subdomains
  includeSubdomains: z.boolean().optional().default(false)
    .describe('Include URLs from subdomains in results. By default, only URLs from the same domain are included. Setting to true will include URLs like blog.example.com when mapping example.com'),
  
  // Optional with default: Maximum depth for HTML link discovery
  maxDepth: z.number().min(1).max(5).optional().default(2)
    .describe('Maximum depth for HTML link discovery (1-5). Only applies when not using sitemapOnly. A depth of 1 only includes links from the starting page, 2 includes links from those pages, etc.'),
  
  // Optional: Filter URLs by path patterns (inclusion)
  includePaths: z.array(z.string()).optional()
    .describe('Only include URLs with paths matching these patterns (e.g., ["/blog/", "/products/"]). URLs must contain at least one of these strings to be included'),
  
  // Optional: Filter URLs by path patterns (exclusion)
  excludePaths: z.array(z.string()).optional()
    .describe('Exclude URLs with paths matching these patterns. URLs containing any of these strings will be excluded from results'),
