...
export const crawl4aiScrapeSchema = z.object({
  url: urlParam,
  formats: z.array(FormatEnum).default(['markdown']).describe('Content formats to extract'),
  actions: z.array(ActionSchema).optional().describe('Actions to perform before scraping'),
  waitFor: z.number().optional().describe('Time to wait for dynamic content'),
  onlyMainContent: z.boolean().optional().describe('Extract only the main content'),
  ...
  removeBase64Images: z.boolean().optional().describe('Remove base64 encoded images'),
