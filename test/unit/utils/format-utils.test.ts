...
describe('Format Utils', () => {
  describe('formatContent', () => {
    it('should pass through content already in MCP format', () => {
      const mcpContent: MCPContent[] = [{ type: 'text', text: 'Already formatted' }];
    ...
    it('should format string content', () => {
      ...
      expect(result).toEqual([{ type: 'text', text: 'Simple string' }]);
    });

    it('should handle primitive types', () => {
      expect(formatContent(123)).toEqual([{ type: 'text', text: '123' }]);

      expect(formatContent(true)).toEqual([{ type: 'text', text: 'true' }]);

      expect(formatContent(null)).toEqual([{ type: 'text', text: 'No content returned.' }]);
    });
    ...
    it('should handle research results without sources', () => {
      ...
      expect(result).toEqual([{ type: 'text', text: 'Research summary' }]);
    });
    ...
    it('should handle string research results', () => {
      ...
      expect(result).toEqual([{ type: 'text', text: 'Simple research result' }]);
    });
    ...
    it('should handle content with markdown property', () => {
      ...
      expect(result).toEqual([{ type: 'text', text: '# Heading\n\nContent with **bold** text' }]);
    });
    ...
    it('should handle content with html property', () => {
      ...
      expect(result).toEqual([
        {
          type: 'html',
          text: '<h1>Heading</h1><p>Content with <strong>bold</strong> text</p>',
        },
      ]);
    });
    ...
    it('should handle content with text property', () => {
      ...
      expect(result).toEqual([{ type: 'text', text: 'Simple text content' }]);
    });
    ...
    it('should default to JSON for complex objects', () => {
      ...
      expect(result).toEqual([{ type: 'json', text: JSON.stringify(content, null, 2) }]);
    });
  });
});
