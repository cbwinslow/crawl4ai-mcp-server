...
describe('Handler Factory', () => {
  ...
  beforeEach(() => {
    ...
    adapter.scrape.mockReset().mockResolvedValue({ success: true, data: 'test' });
    adapter.extract.mockReset().mockResolvedValue({ success: true, data: 'test' });
  ...
  describe('createHandler', () => {
    ...
    it('should transform response when transformResponse is provided', async () => {
      ...
      expect(formatContentSpy).toHaveBeenCalledWith(expect.objectContaining({
        transformed: true
      }));
    });
    ...
    it('should use emptyResponseMessage for empty responses', async () => {
      ...
      expect(result).toEqual({
        content: [{ type: 'text', text: 'No content from https://example.com' }]
      });
    });
  ...
  describe('createStringValidator', () => {
    it('should validate string parameters', () => {
      ...
      expect(() => validator({})).toThrow('url is required and must be a string');
      
      // Should throw for non-string params
      expect(() => validator({ url: 123 })).toThrow('url is required and must be a string');
    });
  ...
  describe('createArrayValidator', () => {
    it('should validate array parameters', () => {
      ...
      // Should throw for missing params
      expect(() => validator({})).toThrow('urls is required and must be an array');
      
      // Should throw for non-array params
      expect(() => validator({ urls: 'not-an-array' })).toThrow('urls is required and must be an array');
    });
